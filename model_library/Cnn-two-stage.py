...
#         self.global_pooling = layers.GlobalMaxPooling2D()
    ...
    def call(self, image_sequences, feature, dv_3h, training):
        ...
        # image_encoder block
        images = tf.reshape(
            image_sequences, [batch_size, height, width, encode_length*channels]
        )      
        encoded_image = self.apply_list_of_layers(
            images, self.encoding_layer, training
        )
        
#         global_pooling_features = self.global_pooling(encoded_image)
        global_pooling_features = layers.GlobalMaxPooling2D()(encoded_image)
