...
def deserialize(serialized_TC_history):
    ...
    images = tf.reshape(
        tf.io.decode_raw(example['images'], tf.float64),
        [history_len, 64, 64, 2]
    )
    ...
    VWSdir = tf.reshape(
        tf.io.decode_raw(example['VWSdir'], tf.float64),
        [history_len]
    )    
    VWSdir = tf.cast(VWSdir, tf.float32)

    TC_spd = tf.reshape(
        tf.io.decode_raw(example['TC_spd'], tf.float64),
        [history_len]
    ) 
...
def image_preprocessing(images, intensity, env_feature, history_len, frame_ID_ascii, VWSdir, TC_spd, rotate_type, input_image_type):
    ...
    return rotated_images, intensity, env_feature, history_len, frame_ID_ascii, TC_spd
...
def get_tensorflow_datasets(
    ...
    for phase, record_path in tfrecord_paths.items():
        ...
        long_enough_histories = TC_histories.filter(
            lambda a, b, c, d, e, f, g: d >= min_history_len
        )
